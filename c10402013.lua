...
function cm.initial_effect(c)
	vgd.VgCard(c)
    vgd.EffectTypeIgnition(c,m,LOCATION_MZONE,cm.op,vgf.DamageCost(1),cm.con)
...
function cm.op(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SelectOption(tp,vgf.Stringid(m,0),vgf.Stringid(m,1))==0 then
		...
		if  c:IsRelateToEffect(e) and c:IsFaceup() then vgf.Sendto(LOCATION_OVERLAY,c) end
...
function cm.checkfilter(c,tp)
    return c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousControler(1-tp) and c:IsControler(1-tp)
end
