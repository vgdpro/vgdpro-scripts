...
function cm.initial_effect(c)
	...
	vgd.action.AbilityAct(c,m,LOCATION_CIRCLE,cm.op,vgf.cost.CounterBlast(1),vgf.con.IsV,nil,1)
	-- 补充一回合一次描述
	-- 【自】【V】：这个单位攻击时，你的指令区中的背面表示的卡有2张以上的话，选择你的指令区中的1张卡正面表示的歌曲卡，将其歌唱，这次战斗中，对手不能将守护者从手牌CALL到G上。
	vgd.action.AbilityAuto(c,m,LOCATION_CIRCLE,EFFECT_TYPE_SINGLE,EVENT_ATTACK_ANNOUNCE,cm.op2,nil,cm.con2)
end
...
function cm.op2(e,tp,eg,ep,ev,re,r,rp)
	...
	e2:SetCode(AFFECT_CODE_DEFENDER_CANNOT_TO_G_CIRCLE)
	e2:SetTargetRange(0,1)
	Duel.RegisterEffect(e2,tp)
	vgf.effect.Reset(c,{e1,e2},EVENT_BATTLED)
end
