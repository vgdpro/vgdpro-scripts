...
function cm.op(e,tp,eg,ep,ev,re,r,rp)
	local ct=e:GetLabel()
	local ct1=vgf.GetMatchingGroupCount(nil,tp,0,LOCATION_CIRCLE,nil)
	if ct>ct1 then ct=ct1 end
	if ct==0 then return end
	local g=vgf.SelectMatchingCard(HINTMSG_LEAVEFIELD,e,tp,nil,tp,0,LOCATION_CIRCLE,1,ct,nil)
